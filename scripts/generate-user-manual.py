...
def build_user_manual(path):
    ...
    add_steps(
        ...
        [
            ...
            "상품 리스트: 검색·수정·삭제",
            "신규 상품 등록 흐름(별도 메뉴)이 있으면 안내에 따라 진행",
        ],
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph("경로: 그룹 마케팅 관리 → 업무관리")
    add_table(
        doc,
        ["메뉴", "용도"],
        [
            ["관리자 등록·리스트", "내부 계정·주문 권한 설정"],
            ["발주서 관리·거래명세서", "주문서 관리와 동일 진입"],
            ["접속통계", "로그인·방문 기록"],
            ["디비사용 통계", "상품·업체·주문 데이터 집계"],
            ["시스템 구조 문서", "관리용 Word/PPT·사용자 매뉴얼 다운로드"],
        ],
    )
