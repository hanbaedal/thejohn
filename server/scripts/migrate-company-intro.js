/**
 * 회사소개(인사말·소개 이미지) — 기존 하드코딩·정적 파일 → staff MongoDB 이전
 * 사용: node scripts/migrate-company-intro.js
 */
...
} = require("../lib/companyIntro");

const REPO_ROOT = path.resolve(__dirname, "../..");
const AEK_GALLERY_DIR = path.join(REPO_ROOT, "img", "company-intro", "aek");
...
function fontCss() {
    if (!fs.existsSync(FONT_REGULAR)) {
        throw new Error("한글 폰트가 없습니다. npm run postinstall 또는 ensure-pdf-font.js를 실행하세요.");
    }
...
async function loadAekGalleryImages() {
    ...
    for (let n = 1; n <= AEK_GALLERY_COUNT; n++) {
        const filePath = path.join(AEK_GALLERY_DIR, n + ".png");
...
async function migrateStaffDoc(col, staff) {
    ...
    const update = {};
    let label = company || staff.id || loginId;
    ...
    if (matchesWooilFoodCompany(company)) {
        ...
        loginId === "ak20140516" ||
        matchesAkSangsaCompany(company)
    ) {
        if (!hasGreeting) {
    ...
    if (!Object.keys(update).length) {
        console.log("[skip]", label, "— 이미 DB에 회사소개 데이터 있음");
        return false;
    }
...
async function main() {
    ...
    for (const staff of staffList) {
        if (await migrateStaffDoc(col, staff)) count++;
    }
    await closeDb();
