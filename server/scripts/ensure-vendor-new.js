/**
 * vendor_new 컬렉션·인덱스 수동 생성 (로컬/배포 DB 확인용)
 * 사용: cd server && node scripts/ensure-vendor-new.js
 */
require("dotenv").config({ path: require("path").join(__dirname, "..", "..", ".env") });
const { MongoClient } = require("mongodb");
const { ensureVendorNewIndexes, COLLECTION } = require("../lib/vendorNew");
...
async function main() {
    ...
    try {
        ...
        await ensureVendorNewIndexes(db);
