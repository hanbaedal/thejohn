...
(function () {
    ...
    function defaultDatesToday() {
        ...
        if ($("slp-date-from") && !$("slp-date-from").value) $("slp-date-from").value = today;

        if ($("slp-date-to") && !$("slp-date-to").value) $("slp-date-to").value = today;
    ...
    function loadVendors() {
        ...
        if (isSupervisor && adminEl && adminEl.value) {

            ledgerOpts.adminStaffId = adminEl.value;

        }

        return Promise.all([

            api.listVendors().catch(function () {

                return [];

            }),

            api.getLedgerVendorCompanies(ledgerOpts).catch(function () {
            ...
            .then(function (results) {
                ...
                var registered = results[0] || [];

                var ledgerNames = results[1] || [];

                var byName = {};

                registered.forEach(function (v) {

                    var name = vendorCompanyName(v);

                    if (name) byName[name.toLowerCase()] = v;

                });

                ledgerNames.forEach(function (name) {
                    ...
                    if (!byName[key]) {
                        ...
                        byName[key] = { vn_company: trimmed, _fromLedger: true };

                    }

                });

                allVendors = Object.keys(byName)
    ...
    function queryParams() {
        ...
        var p = {
            ...
            dateFrom: SR.readDateInput($("slp-date-from")),

            dateTo: SR.readDateInput($("slp-date-to"))
    ...
    function runSearch() {
        ...
        if (!p.dateFrom || !p.dateTo) {
            ...
            return setStatus("조회 기간(시작일·종료일)을 선택해 주세요.", "err");

        }

        if (p.dateFrom > p.dateTo) {

            return setStatus("기간 선택이 올바르지 않습니다. (시작일 ≤ 종료일)", "err");
    ...
    function init() {
        ...
        bindDatePickerOpen($("slp-date-from"));

        bindDatePickerOpen($("slp-date-to"));
